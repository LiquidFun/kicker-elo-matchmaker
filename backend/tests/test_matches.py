...
def _create_user(client, username: str, password: str = "pw12345678") -> int:
    r = client.post(
        "/api/users",
...
def test_non_admin_cannot_change_settings(client, admin_client):
    admin_client.post(
        "/api/users", json={"username": "regular", "display_name": "R", "password": "regpw12345"}
    )
    admin_client.post("/api/auth/logout")
    admin_client.post("/api/auth/login", json={"username": "regular", "password": "regpw12345"})
